...
function createOAuthClient(sessionId: string) {
...
export async function fetchThreads(sessionId: string): Promise<RawThread[]> {
  const auth = createOAuthClient(sessionId);
  ...
  for (const chunk of chunks) {
    ...
    for (const thread of threadDetails) {
      ...
      results.push({ id: thread.id, subject, sender, snippet, timestamp, unread });
    }
  }
...
export async function fetchThreadsByIds(sessionId: string, threadIds: string[]): Promise<RawThread[]> {
  const auth = createOAuthClient(sessionId);
  const gmail = google.gmail({ version: 'v1', auth });
...
export async function fetchThreadBody(sessionId: string, threadId: string): Promise<string> {
  const auth = createOAuthClient(sessionId);
  const gmail = google.gmail({ version: 'v1', auth });
...
function extractTextFromPayload(payload: any | null): string {
  ...
  for (const part of parts) {
    ...
    if (part.mimeType === 'text/html' && part.body?.data) {
      htmlFallback = stripHtmlTags(Buffer.from(part.body.data, 'base64url').toString('utf-8'));
    }
