...
export default function SettingsPage({
      ...
      prev.map((r) =>
        r.name === name
          ? {
              ...r,
              hint,
              saved: hint === (bucketHints[name] || getDefaultHint(name)),
            }
          : r,
      ),
    );
  ...
  const handleSaveBucket = (name: string, hint: string) => {
    ...
    setRows((prev) =>
      prev.map((r) => (r.name === name ? { ...r, saved: true } : r)),
    );
  };
  ...
  const isDupeNew = buckets.some(
    (b) => b.toLowerCase() === sanitizedNew.toLowerCase(),
  );
        ...
        <div className="max-w-2xl mx-auto px-6 py-8">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Each bucket has a classification prompt that tells the AI what
              emails belong there. Edit and save individually.
            </p>
          </div>
